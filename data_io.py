...
#sup1 data
def read_sup1_train_pairs():
    train_path = get_paths()["sup1_train_pairs_path"]
    return parse_dataframe(pd.read_csv(train_path, index_col="SampleID"))

def read_sup1_train_target():
    path = get_paths()["sup1_train_target_path"]
    df = pd.read_csv(path, index_col="SampleID")
    df = df.rename(columns = dict(zip(df.columns, ["Target", "Details"])))
    return df

def read_sup1_train_info():
    path = get_paths()["sup1_train_info_path"]
    return pd.read_csv(path, index_col="SampleID")

#sup2 data
def read_sup2_train_pairs():
    train_path = get_paths()["sup2_train_pairs_path"]
    return parse_dataframe(pd.read_csv(train_path, index_col="SampleID"))

def read_sup2_train_target():
    path = get_paths()["sup2_train_target_path"]
    df = pd.read_csv(path, index_col="SampleID")
    df = df.rename(columns = dict(zip(df.columns, ["Target", "Details"])))
    return df

def read_sup2_train_info():
    path = get_paths()["sup2_train_info_path"]
    return pd.read_csv(path, index_col="SampleID")

#sup3 data
def read_sup3_train_pairs():
    train_path = get_paths()["sup3_train_pairs_path"]
    return parse_dataframe(pd.read_csv(train_path, index_col="SampleID"))

def read_sup3_train_target():
    path = get_paths()["sup3_train_target_path"]
    df = pd.read_csv(path, index_col="SampleID")
    df = df.rename(columns = dict(zip(df.columns, ["Target", "Details"])))
    return df

def read_sup3_train_info():
    path = get_paths()["sup3_train_info_path"]
    return pd.read_csv(path, index_col="SampleID")
